...
#   nohup caffeinate -is bash scripts/orbital/extj2/t15d_dagger_campaign.sh \
#       > /tmp/t15d_stdout.log 2>&1 &
#
...
#     re-run at T15D_LR=2e-3; if that also splits the difference, run a STAGED
#     LR (2e-3 for the first ~30M to re-acquire the wides, then 1e-3 to
#     consolidate) via T15D_LR + T15D_LR2/T15D_LR_SWITCH. Staged is second, not
...
ROOT=${T15D_ROOT:-$MAIN/models/t3/t15c_dagger_final.pt}
PROG=/tmp/t15d_progress.log
JSON_DIR=$MAIN/web_data/results/t15d_dagger
EPS=200
EVAL_SEED=123
STEPS=${T15D_STEPS:-75000000}
MID_STEPS=${T15D_MID_STEPS:-50000000}
WATCHDOG_S=900
NAV_MAX_TICKS=${T15D_NAV_MAX_TICKS:-120}
...
ACQ_LAMBDA=${T15D_ACQ_LAMBDA:-0.20}
ACQ_LAMBDA_END=${T15D_ACQ_LAMBDA_END:-0.20}
ACQ_DECAY=${T15D_ACQ_DECAY:-0}
DEF_LAMBDA=${T15D_DEF_LAMBDA:-0.02}
ANCHOR_MB=${T15D_ANCHOR_MB:-4}
ACQ_DATA=${T15D_ACQ_DATA:-$MAIN/models/t15/anchor_w1_dagger3.pt}
ACQ_CKPT=${T15D_ACQ_CKPT:-$MAIN/models/t3/w1nav_child.pt}
DEF_DATA=${T15D_DEF_DATA:-$MAIN/models/t15/anchor_tight_k0.pt}
DEF_CKPT=${T15D_DEF_CKPT:-$MAIN/models/t3/t13b_anchor_final.pt}
NAV_MAX_TICKS=${T15D_NAV_MAX_TICKS:-0}
FILTER_IMPL=${T15D_FILTER_IMPL:-c}
...
LR2=${T15D_LR2:-}                 # staged-LR second leg (unset = single LR)
LR_SWITCH=${T15D_LR_SWITCH:-30000000}

# Tripwire: ADVISORY by default, and that is a deliberate choice — see below.
TRIP_AT=${T15D_TRIP_AT:-25000000}
TRIP_EPS=${T15D_TRIP_EPS:-20}
TRIP_FLOOR=${T15D_TRIP_FLOOR:-46}   # half the root's 92.5% tight
TRIP_FATAL=${T15D_TRIP_FATAL:-0}
ENT_FLOOR=${T15D_ENT_FLOOR:-0.45}   # measured operating point 0.71; see red-team (c)

T15D_SEED=${T15D_SEED:-42}
SEED_SFX=""
[ "$T15D_SEED" != "42" ] && SEED_SFX="_s${T15D_SEED}"
STAGES=${T15D_STAGES:-0,1,2,3}
want() { [[ ",$STAGES," == *",$1,"* ]]; }
...
EPOCH_SLACK=${T15D_EPOCH_SLACK:-3}
ARM_DIR="$PUF/experiments_t15d_dagger/consol${SEED_SFX}"
final_ckpt() {
...
preflight() {
    ...
    if [ "$(shasum -a256 "$DEF_CKPT" | cut -d" " -f1)" = \
        ...
        python3 "$ROOTGATE" --root "$DEF_CKPT" --cell TIGHT_5k1 \
            ...
            --filter-impl "$FILTER_IMPL" > /tmp/t15d_defgate.log 2>&1 \
          || { say "ABORT: defense teacher fails root_gate on TIGHT_5k1"; return 1; }
        say "  RESULT defense-teacher root_gate: $(grep -oE '[0-9]+/[0-9]+ root gates pass' /tmp/t15d_defgate.log | tail -1)"
    fi
...
anchor() {
    ...
    cd "$MAIN" || return 1
    python3 "$VERIFY" --stage a1,a2,a3,a4,a5 --eps 100 > /tmp/t15d_anchor.log 2>&1
    local ra=$?
    say "  RESULT anchors rc=$ra $(grep -oE '[0-9]+/[0-9]+ checks pass' /tmp/t15d_anchor.log | tail -1)"
    grep -E '^\s+\[FAIL\]' /tmp/t15d_anchor.log | while read -r l; do say "    $l"; done
    python3 "$GATES" > /tmp/t15d_gates.log 2>&1
    local rg=$?
    say "  RESULT t11 gates rc=$rg $(grep -oE '[0-9]+/[0-9]+ gates pass' /tmp/t15d_gates.log | tail -1)"
    grep -E '^\s+\[FAIL\]' /tmp/t15d_gates.log | while read -r l; do say "    $l"; done
    grep -E '^\s+\[(PASS|FAIL)\] G9' /tmp/t15d_gates.log | while read -r l; do say "    $l"; done
...
# ═══════════════════════════════════════════════════════════════════════════
say "=========== T15d DAgger iteration-3 campaign start (pid $$) ==========="
say "QUESTION: iteration 3. CE-tracks-yield is FALSIFIED (CE halved, yield held),"
say "          novelty is flat, and success cap-usage p90 0.51->0.70 predicts the"
say "          frontier exhausts after ~iteration 4. Expect a buy, then the bend."
...
say "          W1 44.5%  TIGHT 35.3%  wides 20.2% (verified at MEAN dec/ep 290, not median)"
...
say "          teacher still competent (H 0.84 vs flat 3.43); novelty flat at 1.13"
say "baseline: root battery wides 95-99.5, TIGHT 84.5, W1 59.0 (iters bought +13.5, +14.0)"
say "STATE:    all rows MEAN-ELEMENT claims under J2; rel slip 83 m / 0.094 m/s per orbit at 5 km"
say "stages requested: $STAGES   seed $T15D_SEED${SEED_SFX:+ (suffixed)}"
preflight || exit 1
...
PYJ
    ...
    python3 -m pufferlib.pufferl train puffer_orbital_nav \
        --train.device cpu --train.total-timesteps "$STEPS" --train.seed "$T15D_SEED" \
        --train.data-dir "$ARM_DIR" --load-model-path "$ROOT" \
        $LR_ARGS --train.checkpoint-interval "${T15D_CKPT_INT:-20}" \
        ...
        --env.t11-mixture 5 \
        --wandb --wandb-project orbital-rl --wandb-group "t15d-dagger${SEED_SFX}" \
        --tag "t15b_dagger${SEED_SFX}" > "/tmp/t15d_consol${SEED_SFX}_train.log" 2>&1 &
    PID=$!; say "  trainer pid $PID"
    TL="/tmp/t15d_consol${SEED_SFX}_train.log"
    ...
    while kill -0 "$PID" 2>/dev/null; do
      ...
      # measurement. So this records and warns; set T15D_TRIP_FATAL=1 to kill.
...
if want 3; then
  ...
  if [ -z "$CK" ]; then say "  SKIP: no final checkpoint"; else
    ...
    say "     this is red-team (a), NOT a capacity result. Fallback T15D_LR=2e-3,"
    say "     then staged (T15D_LR2/T15D_LR_SWITCH)."
...
say "=========== t15d-dagger campaign COMPLETE ==========="
