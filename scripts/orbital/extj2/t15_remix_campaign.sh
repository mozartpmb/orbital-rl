...
#   nohup caffeinate -is bash scripts/orbital/extj2/t15_remix_campaign.sh \
#       > /tmp/t15_stdout.log 2>&1 &
#
...
#     re-run at T15_LR=2e-3; if that also splits the difference, run a STAGED
#     LR (2e-3 for the first ~30M to re-acquire the wides, then 1e-3 to
#     consolidate) via T15_LR + T15_LR2/T15_LR_SWITCH. Staged is second, not
...
ROOT=${T15_ROOT:-$MAIN/models/t3/t13b_anchor_final.pt}
PROG=/tmp/t15_progress.log
JSON_DIR=$MAIN/web_data/results/t15_remix
EPS=200
EVAL_SEED=123
STEPS=${T15_STEPS:-100000000}
MID_STEPS=${T15_MID_STEPS:-50000000}
WATCHDOG_S=900
NAV_MAX_TICKS=${T15_NAV_MAX_TICKS:-120}
...
ACQ_LAMBDA=${T15_ACQ_LAMBDA:-0.05}
ACQ_LAMBDA_END=${T15_ACQ_LAMBDA_END:-0.01}
ACQ_DECAY=${T15_ACQ_DECAY:-30000000}
DEF_LAMBDA=${T15_DEF_LAMBDA:-0.02}
ANCHOR_MB=${T15_ANCHOR_MB:-4}
ACQ_DATA=${T15_ACQ_DATA:-$MAIN/models/t15/anchor_w1_k0.pt}
ACQ_CKPT=${T15_ACQ_CKPT:-$MAIN/models/t3/w1nav_child.pt}
DEF_DATA=${T15_DEF_DATA:-$MAIN/models/t15/anchor_tight_k0.pt}
DEF_CKPT=${T15_DEF_CKPT:-$MAIN/models/t3/t13b_anchor_final.pt}
NAV_MAX_TICKS=${T15_NAV_MAX_TICKS:-0}
FILTER_IMPL=${T15_FILTER_IMPL:-c}
...
LR2=${T15_LR2:-}                 # staged-LR second leg (unset = single LR)
LR_SWITCH=${T15_LR_SWITCH:-30000000}

# Tripwire: ADVISORY by default, and that is a deliberate choice — see below.
TRIP_AT=${T15_TRIP_AT:-25000000}
TRIP_EPS=${T15_TRIP_EPS:-20}
TRIP_FLOOR=${T15_TRIP_FLOOR:-46}   # half the root's 92.5% tight
TRIP_FATAL=${T15_TRIP_FATAL:-0}
ENT_FLOOR=${T15_ENT_FLOOR:-0.45}   # measured operating point 0.71; see red-team (c)

T15_SEED=${T15_SEED:-42}
SEED_SFX=""
[ "$T15_SEED" != "42" ] && SEED_SFX="_s${T15_SEED}"
STAGES=${T15_STAGES:-0,1,2,3}
want() { [[ ",$STAGES," == *",$1,"* ]]; }
...
EPOCH_SLACK=${T15_EPOCH_SLACK:-3}
ARM_DIR="$PUF/experiments_t15_remix/consol${SEED_SFX}"
final_ckpt() {
...
anchor() {
    ...
    cd "$MAIN" || return 1
    python3 "$VERIFY" --stage a1,a2,a3,a4,a5 --eps 100 > /tmp/t15_anchor.log 2>&1
    local ra=$?
    say "  RESULT anchors rc=$ra $(grep -oE '[0-9]+/[0-9]+ checks pass' /tmp/t15_anchor.log | tail -1)"
    grep -E '^\s+\[FAIL\]' /tmp/t15_anchor.log | while read -r l; do say "    $l"; done
    python3 "$GATES" > /tmp/t15_gates.log 2>&1
    local rg=$?
    say "  RESULT t11 gates rc=$rg $(grep -oE '[0-9]+/[0-9]+ gates pass' /tmp/t15_gates.log | tail -1)"
    grep -E '^\s+\[FAIL\]' /tmp/t15_gates.log | while read -r l; do say "    $l"; done
    grep -E '^\s+\[(PASS|FAIL)\] G9' /tmp/t15_gates.log | while read -r l; do say "    $l"; done
...
PYJ
    ...
    python3 -m pufferlib.pufferl train puffer_orbital_nav \
        --train.device cpu --train.total-timesteps "$STEPS" --train.seed "$T15_SEED" \
        --train.data-dir "$ARM_DIR" --load-model-path "$ROOT" \
        $LR_ARGS --train.checkpoint-interval "${T15_CKPT_INT:-20}" \
        ...
        --env.t11-mixture 3 \
        --wandb --wandb-project orbital-rl --wandb-group "t15-remix${SEED_SFX}" \
        --tag "t15_remix${SEED_SFX}" > "/tmp/t15_consol${SEED_SFX}_train.log" 2>&1 &
    PID=$!; say "  trainer pid $PID"
    TL="/tmp/t15_consol${SEED_SFX}_train.log"
    ...
    while kill -0 "$PID" 2>/dev/null; do
      ...
      # measurement. So this records and warns; set T15_TRIP_FATAL=1 to kill.
...
if want 3; then
  ...
  if [ -z "$CK" ]; then say "  SKIP: no final checkpoint"; else
    ...
    say "     this is red-team (a), NOT a capacity result. Fallback T15_LR=2e-3,"
    say "     then staged (T15_LR2/T15_LR_SWITCH)."
...
say "=========== t15-remix campaign COMPLETE ==========="
