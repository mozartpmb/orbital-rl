...
#   nohup caffeinate -is bash scripts/orbital/extj2/t13b_anchor_campaign.sh \
#       > /tmp/t13b_stdout.log 2>&1 &
#
...
#     re-run at T13B_LR=2e-3; if that also splits the difference, run a STAGED
#     LR (2e-3 for the first ~30M to re-acquire the wides, then 1e-3 to
#     consolidate) via T13B_LR + T13B_LR2/T13B_LR_SWITCH. Staged is second, not
...
PROG=/tmp/t13b_progress.log
JSON_DIR=$MAIN/web_data/results/t13b_anchor
EPS=200
EVAL_SEED=123
STEPS=${T13B_STEPS:-100000000}
MID_STEPS=${T13B_MID_STEPS:-50000000}
WATCHDOG_S=900
NAV_MAX_TICKS=${T13B_NAV_MAX_TICKS:-120}
...
ANCHOR_LAMBDA=${T13B_ANCHOR_LAMBDA:-0.02}
ANCHOR_DATA=${T13B_ANCHOR_DATA:-$MAIN/models/t13b/t11b_anchor_tight.pt}
ANCHOR_CKPT=${T13B_ANCHOR_CKPT:-$ROOT}
ANCHOR_MB=${T13B_ANCHOR_MB:-4}
LR2=${T13B_LR2:-}                 # staged-LR second leg (unset = single LR)
LR_SWITCH=${T13B_LR_SWITCH:-30000000}

# Tripwire: ADVISORY by default, and that is a deliberate choice — see below.
TRIP_AT=${T13B_TRIP_AT:-25000000}
TRIP_EPS=${T13B_TRIP_EPS:-20}
TRIP_FLOOR=${T13B_TRIP_FLOOR:-46}   # half the root's 92.5% tight
TRIP_FATAL=${T13B_TRIP_FATAL:-0}
ENT_FLOOR=${T13B_ENT_FLOOR:-0.45}   # measured operating point 0.71; see red-team (c)

T13B_SEED=${T13B_SEED:-42}
SEED_SFX=""
[ "$T13B_SEED" != "42" ] && SEED_SFX="_s${T13B_SEED}"
STAGES=${T13B_STAGES:-0,1,2,3}
want() { [[ ",$STAGES," == *",$1,"* ]]; }
...
EPOCH_SLACK=${T13B_EPOCH_SLACK:-3}
ARM_DIR="$PUF/experiments_t13b_anchor/consol${SEED_SFX}"
final_ckpt() {
...
anchor() {
    ...
    cd "$MAIN" || return 1
    python3 "$VERIFY" --stage a1,a2,a3,a4,a5 --eps 100 > /tmp/t13b_anchor.log 2>&1
    local ra=$?
    say "  RESULT anchors rc=$ra $(grep -oE '[0-9]+/[0-9]+ checks pass' /tmp/t13b_anchor.log | tail -1)"
    grep -E '^\s+\[FAIL\]' /tmp/t13b_anchor.log | while read -r l; do say "    $l"; done
    python3 "$GATES" > /tmp/t13b_gates.log 2>&1
    local rg=$?
    say "  RESULT t11 gates rc=$rg $(grep -oE '[0-9]+/[0-9]+ gates pass' /tmp/t13b_gates.log | tail -1)"
    grep -E '^\s+\[FAIL\]' /tmp/t13b_gates.log | while read -r l; do say "    $l"; done
    grep -E '^\s+\[(PASS|FAIL)\] G9' /tmp/t13b_gates.log | while read -r l; do say "    $l"; done
...
# ═══════════════════════════════════════════════════════════════════════════
say "=========== T13b-anchor campaign start (pid $$) ==========="
...
say "STATE:    tight rows are MEAN-ELEMENT claims (relative mean-vs-osc slip 83 m / 0.094 m/s per orbit at 5 km sep)"
say "stages requested: $STAGES   seed $T13B_SEED${SEED_SFX:+ (suffixed)}"
preflight || exit 1
...
if want 1; then
  ...
  if [ -n "$(final_ckpt "$ARM_DIR" "$WANT_EP")" ]; then
    ...
    python3 -m pufferlib.pufferl train puffer_orbital_nav \
        --train.device cpu --train.total-timesteps "$STEPS" --train.seed "$T13B_SEED" \
        --train.data-dir "$ARM_DIR" --load-model-path "$ROOT" \
        $LR_ARGS --train.checkpoint-interval "${T13B_CKPT_INT:-20}" \
        --train.anchor-lambda "$ANCHOR_LAMBDA" \
        --train.anchor-data "$ANCHOR_DATA" \
        --train.anchor-ckpt "$ANCHOR_CKPT" \
        ...
        --env.t11-mixture 2 \
        --wandb --wandb-project orbital-rl --wandb-group "t13b-anchor${SEED_SFX}" \
        --tag "t13b_anchor${SEED_SFX}" > "/tmp/t13b_consol${SEED_SFX}_train.log" 2>&1 &
    PID=$!; say "  trainer pid $PID"
    TL="/tmp/t13b_consol${SEED_SFX}_train.log"
    ...
    while kill -0 "$PID" 2>/dev/null; do
      ...
      # measurement. So this records and warns; set T13B_TRIP_FATAL=1 to kill.
...
if want 3; then
  ...
  if [ -z "$CK" ]; then say "  SKIP: no final checkpoint"; else
    ...
    say "     this is red-team (a), NOT a capacity result. Fallback T13B_LR=2e-3,"
    say "     then staged (T13B_LR2/T13B_LR_SWITCH)."
...
say "=========== t13b-anchor campaign COMPLETE ==========="
